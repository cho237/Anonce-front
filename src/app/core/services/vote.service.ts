...
export class VoteService {
  ...
  getAll(isActive: boolean): Observable<VoteResponse<VoteListRes[]>> {
    return this.http.get<VoteResponse<VoteListRes[]>>(
  ...
  cast(
    ...
    candidateId: string,
    password: string
  ): Observable<VoteResponse<Vote>> {
    return this.http.post<VoteResponse<Vote>>(`${this.apiUrl}/voter`, {
      voteId,
      candidateId,
      password,
    });
  ...
  voters(voteId: string): Observable<VoteResponse<Voter[]>> {
    return this.http.get<VoteResponse<Voter[]>>(
      `${this.apiUrl}/${voteId}/voters`
    );
  }
